...
| [ADR-037](ADR-037-deterministic-correlation-path-for-repo-only-input.md) | Proposed | A second, repo-only entry point bypassing the LLM planner entirely for deterministic PR/Sentry-issue/Jira-ticket correlation. Phases 1-3 implemented and live-verified: `HttpSentrySource.list_open_issues` (Sentry's `Link`-header cursor pagination), `.get_linked_jira_key` (`GET /organizations/{org}/issues/{id}/integrations/`'s `externalIssues[0].key`), `.get_issue_commit_sha` (the issue-detail response's embedded `firstRelease`, not `get_deployment_evidence`), and `scan_repository_for_correlations` (a `MAX_ISSUES_PER_SCAN`-capped, per-issue-failure-isolated orchestrator producing a flat correlation list with an explicit `ok`/`partial`/`failed` status per issue). Phase 4 (the HTTP entry point) still planned |
