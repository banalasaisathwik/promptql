...
| [ADR-037](ADR-037-deterministic-correlation-path-for-repo-only-input.md) | Accepted; amended by ADR-038 | A second, repo-only entry point bypassing the LLM planner entirely for deterministic PR/Sentry-issue/Jira-ticket correlation. All 4 phases implemented and live-verified: `HttpSentrySource.list_open_issues` (Sentry's `Link`-header cursor pagination), `.get_linked_jira_key` (`GET /organizations/{org}/issues/{id}/integrations/`'s `externalIssues[0].key`), `.get_issue_commit_sha` (the issue-detail response's embedded `firstRelease`, not `get_deployment_evidence`), `scan_repository_for_correlations` (a `MAX_ISSUES_PER_SCAN`-capped, per-issue-failure-isolated orchestrator producing a flat correlation list with an explicit `ok`/`partial`/`failed` status per issue), and `POST /v1/correlation-scans` (requires an authenticated user's own Sentry+GitHub credentials, no anonymous/demo fallback; synchronous, not async/SSE). Its "no hypothesis, no rendering through `render_grounded_result`" decision sentence is superseded by ADR-038; the "zero coupling to the planner/executor" invariant is unchanged and still holds. |
| [ADR-038](ADR-038-grounded-reasoning-for-the-deterministic-correlation-scan.md) | Accepted | Per Sentry issue, once ADR-037's deterministic correlation finishes, reuse the adaptive path's own `TypedLLMHypothesisGenerator` → `DeterministicHypothesisValidator` → `TypedLLMCodeDiagnoser` → `DeterministicCodeFindingValidator` → `build_developer_recommendations` pipeline directly (no planner/executor coupling added); adds an additive `IssueCorrelationResult.analysis: IssueGroundedAnalysis` field with its own `IssueAnalysisStatus` vocabulary, deterministically gated before any LLM call, with per-issue failure isolation; also fixes `CodeContextBuilder`/`DeterministicCodeFindingValidator` to recognize ADR-036's commit-scoped evidence, which they previously ignored entirely |
