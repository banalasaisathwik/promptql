...
| [ADR-034](ADR-034-multi-user-auth-and-per-user-credential-storage.md) | Accepted | Phase 1 only: minimal `users` table, Argon2id password hashing (`argon2-cffi`), a signed/httponly/secure/samesite=lax session cookie (`itsdangerous`), and register/login/logout routes; `get_current_user` is built but not yet wired into any connector dependency or existing route, and per-user credential storage is deferred to a future phase |
