...
export function InvestigationDashboard({ run }: { run: InvestigationRun }) {
  ...
  return (
    <>
      ...
      <section className="run-card" aria-labelledby="evidence-title"><div className="run-section-heading"><div><p className="step-label">Evidence</p><h2 id="evidence-title">Normalized observations</h2></div><span>{state?.evidence_content.length ?? 0}</span></div>{state?.evidence_content.length ? <ul className="plain-list">{state.evidence_content.map((item) => <li key={item.evidence_id}><strong>{evidenceSummary(item)}</strong><small>{item.source} · {item.evidence_id}</small></li>)}</ul> : <p className="run-empty">Evidence will appear as tools return normalized observations.</p>}</section>
