...
class EvidenceModelTests(unittest.TestCase):
    ...
    def test_each_initial_source_kind_content_pair_is_valid(self) -> None:
        cases = (
            ...
            Evidence(
                evidence_id="evidence:commit-changed-file",
                source="github",
                kind="commit_changed_file",
                provenance=provenance(f"github:acme/checkout:commit:{'a' * 40}:file:services/checkout.py"),
            ...
            Evidence(
                evidence_id="evidence:commit-diff-hunk",
                source="github",
                kind="commit_diff_hunk",
                provenance=provenance(f"github:acme/checkout:commit:{'a' * 40}:hunk:1"),
