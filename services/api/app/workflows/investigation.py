...
from app.explanations import LLMClient
...
from app.investigations.planning import PlanValidator
...
class InvestigationWorkflowService:
    ...
    async def continue_persisted_run(self, pending: InvestigationRun) -> InvestigationRun:
        ...
        plan = _build_static_plan(running.request)
        if plan is None:
            return await self._complete_without_evidence(running, started_at)
        ...
        validation = PlanValidator(registry).validate(plan, registry.list())
        ...
        running_with_plan = running.model_copy(
            update={"state": self._pending_state(plan)}
        )
        self._repository.save(running_with_plan)
        ...
        execution = await self._add_failure_location_evidence(
            execution,
            running.request,
        )
        state = self._snapshot_from_execution(execution, plan)

        validated_hypotheses = ()
        rejected_count = 0
        termination_reason = _grounded_reason(execution.termination_reason.value)
        try:
            ...
            validation_result = DeterministicHypothesisValidator().validate(
                generated.candidates, execution.facts
            )
...
def _grounded_reason(reason: str) -> GroundedTerminationReason:
    return {
        ...
        "budget_exhausted": GroundedTerminationReason.BUDGET_EXHAUSTED,
